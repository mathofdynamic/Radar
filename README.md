...
Cloudflare Cron every minute
  ...
  -> Nebula /v1/chat/completions, model=auto
...
The API key is never committed. Nebula owns provider routing and fallback. Radar does not store provider credentials or rotate provider keys. Workers AI remains only for optional image cover generation.
