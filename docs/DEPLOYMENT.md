...
The Worker configuration uses `NEBULA_INTELLIGENCE_MODEL=@cf/zai-org/glm-4.7-flash` for structured event intelligence and `NEBULA_EDITORIAL_MODEL=auto` for editorial text. Intelligence requests allow `NEBULA_INTELLIGENCE_TIMEOUT_MS=95000` so Nebula can complete its own provider fallback. The API key must be stored as a Worker secret and never committed:
